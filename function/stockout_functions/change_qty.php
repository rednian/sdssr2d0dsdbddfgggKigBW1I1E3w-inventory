...
if (isset($_GET['change'])) {
  ...
  if ($type == "minus") {
    ...
    $prod_qty = get_main_qty($temp_id);
    $qty = get_qty($temp_id);
    $newQty = $qty + 1;
    if ($newQty <= $prod_qty) {
      update($temp_id, $newQty);
      LoadItems();
    } elseif ($newQty > $prod_qty) {
      LoadItems();
    }
...
function get_main_qty($temp_id)
{
  global $db;
  global $user;
  $sql = "SELECT prod_total_quantity.quantity
				from prod_total_quantity,temp_purchase 
				where temp_purchase.b_id = prod_total_quantity.b_id 
				and temp_purchase.temp_id = {$temp_id}";
...
function get_qty($temp_id)
{
  global $user;
  global $db;
  $sql = "SELECT qty from temp_purchase where temp_id = {$temp_id} and user_id = {$user}";
  $result = $db->query($sql);
